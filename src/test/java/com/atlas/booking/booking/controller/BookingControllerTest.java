...
class BookingControllerTest {
    ...
    void createBooking_newBooking_returns_201() throws Exception {
        when(bookingService.createBooking(eq(BookingTestData.IDEMPOTENCY_KEY), any(CreateBookingRequest.class)))
    ...
    void getBooking_found_returns_200() throws Exception {
        when(bookingService.getBooking(BookingTestData.BOOKING_ID)).thenReturn(BookingTestData.aBookingResponse());

        mvc.perform(get(BASE_URL + "/{bookingId}", BookingTestData.BOOKING_ID).with(jwt()))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.bookingId").value(BookingTestData.BOOKING_ID.toString()))
                .andExpect(jsonPath("$.status").value("PENDING"));
    ...
    void getBooking_unauthenticated_returns_401() throws Exception {
        mvc.perform(get(BASE_URL + "/{bookingId}", BookingTestData.BOOKING_ID)).andExpect(status().isUnauthorized());
    }
    ...
    void cancelBooking_withoutBody_returns_200() throws Exception {
        when(bookingService.cancelBooking(
                        eq(BookingTestData.CANCELLATION_IDEMPOTENCY_KEY), eq(BookingTestData.BOOKING_ID), eq(null)))
                .thenReturn(BookingTestData.aBookingResponseWithStatus(ApiBookingStatus.CONFIRMED));
