...
class StateTransitionGuardTest {
    ...
    void allowedTransitions_do_not_throw(String from, String to) {
        assertThatCode(() -> StateTransitionGuard.assertAllowed(BookingStatus.valueOf(from), BookingStatus.valueOf(to)))
    ...
    void forbiddenTransitions_throw_InvalidStateTransitionException(String from, String to) {
        assertThatThrownBy(() ->
                        StateTransitionGuard.assertAllowed(BookingStatus.valueOf(from), BookingStatus.valueOf(to)))
    ...
    void terminal_FAILED_has_no_outgoing_transitions() {
        for (BookingStatus target : BookingStatus.values()) {
            assertThatThrownBy(() -> StateTransitionGuard.assertAllowed(BookingStatus.FAILED, target))
                    .isInstanceOf(InvalidStateTransitionException.class);
        }
    }
    ...
    void terminal_EXPIRED_has_no_outgoing_transitions() {
        for (BookingStatus target : BookingStatus.values()) {
            assertThatThrownBy(() -> StateTransitionGuard.assertAllowed(BookingStatus.EXPIRED, target))
                    .isInstanceOf(InvalidStateTransitionException.class);
        }
    }
    ...
    void terminal_CANCELLED_has_no_outgoing_transitions() {
        for (BookingStatus target : BookingStatus.values()) {
            assertThatThrownBy(() -> StateTransitionGuard.assertAllowed(BookingStatus.CANCELLED, target))
                    .isInstanceOf(InvalidStateTransitionException.class);
        }
    }
    ...
    void paymentTransition_from_PENDING_is_premature(String to) {
        assertThatThrownBy(() ->
                        StateTransitionGuard.assertPaymentTransition(BookingStatus.PENDING, BookingStatus.valueOf(to)))
                .isInstanceOf(PrematureSagaEventException.class);
    }

    @ParameterizedTest(name = "INVENTORY_RESERVED → {0} (payment) is allowed")
    @CsvSource({"CONFIRMED", "FAILED", "EXPIRED"})
    void paymentTransition_from_INVENTORY_RESERVED_is_allowed(String to) {
        assertThatCode(() -> StateTransitionGuard.assertPaymentTransition(
    ...
    @CsvSource({"CONFIRMED", "FAILED", "EXPIRED", "CANCELLED"})
    void paymentTransition_from_terminal_is_illegal(String from) {
        assertThatThrownBy(() -> StateTransitionGuard.assertPaymentTransition(
